...
# First stage 
fs1 <- feols(ihs(wealth_defl) ~ father_politician | party_granular + law, cluster=~b1_nummer, data = electoral)
# IV Blank
iv1 <- feols(vote ~ 1 | law + party_granular | ihs(wealth_defl) ~ father_politician, cluster=~b1_nummer, data = electoral)
# First stage - select controls
fs2 <- feols(ihs(wealth_defl) ~ father_politician + industry + services + catholic_pct + hervormd_pct | party_granular + law, cluster=~b1_nummer, data=electoral)
# IV - select controls
iv2 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct | party_granular + law | ihs(wealth_defl) ~ father_politician, cluster=~b1_nummer, data=electoral)
...
iv3 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct + 
             ...
             | party_granular + law | ihs(wealth_defl) ~ father_politician, cluster=~b1_nummer, data = electoral)
