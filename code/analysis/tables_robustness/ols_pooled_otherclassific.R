...
suffrage_basic <-  feols(vote ~ ihs(wealth_defl) | party_simple + law, cluster=~b1_nummer, data=electoral)
fiscal_basic <- feols(vote ~ ihs(wealth_defl) | party_simple + law, cluster=~b1_nummer, data=fiscal)

# First, main model split up
m1 <- feols(vote ~ ihs(wealth_defl):category | party_simple + law, cluster=~b1_nummer, data=data)
# Suffrage (First three panels)
m2 <- feols(vote ~ ihs(wealth_defl) + industry + services + catholic_pct + hervormd_pct | party_simple + law, cluster=~b1_nummer, data=electoral)
m3 <- feols(vote ~ ihs(wealth_defl) + industry + services + catholic_pct + hervormd_pct + 
                income_tax + vermogen_tax + total_pers_taxes + amount_of_strikes | party_simple + law, cluster=~b1_nummer, data=electoral)
m4 <- feols(vote ~ ihs(wealth_defl) + industry + services + catholic_pct + hervormd_pct + 
            ...
            | party_simple + law, cluster=~b1_nummer, data = electoral)
# Fiscal (Second three panels)
m5 <- feols(vote ~ ihs(wealth_defl) + industry + services + catholic_pct + hervormd_pct | party_simple + law, cluster=~b1_nummer, data=fiscal)
m6 <- feols(vote ~ ihs(wealth_defl) + industry + services + catholic_pct + hervormd_pct + 
                income_tax + vermogen_tax + total_pers_taxes + amount_of_strikes | party_simple + law, cluster=~b1_nummer, data=fiscal)
m7 <- feols(vote ~ ihs(wealth_defl) + industry + services + catholic_pct + hervormd_pct + 
                income_tax + vermogen_tax + total_pers_taxes + tenure + 
                soc_share + soc_dum + vote_share + vote_share_nc + turnout + amount_of_strikes + dsle
            | party_simple + law, cluster=~b1_nummer, data = fiscal)
