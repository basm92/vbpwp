...
# First stage 
fs1 <- feols(log(1+wealth_defl) ~ father_politician | party_granular + law, cluster=~b1_nummer, data = fiscal)
# IV Blank
iv1 <- feols(vote ~ 1 | law + party_granular | log(1+wealth_defl) ~ father_politician, cluster=~b1_nummer, data = fiscal)
# First stage - select controls
fs2 <- feols(log(1+wealth_defl) ~ father_politician + industry + services + catholic_pct + hervormd_pct | party_granular + law, cluster=~b1_nummer, data=fiscal)
# IV - select controls
iv2 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct | party_granular + law | log(1+wealth_defl) ~ father_politician, cluster=~b1_nummer, data=fiscal)
# First stage - full controls
fs3 <- feols(log(1+wealth_defl) ~ father_politician + industry + services + catholic_pct + hervormd_pct + 
...
iv3 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct + 
             ...
             | party_granular + law | log(1+wealth_defl) ~ father_politician, cluster=~b1_nummer, data = fiscal)

fstats <- map_chr(list(iv1, iv2, iv3), ~ fitstat(.x, 'ivwald1') |> pluck(1) |> pluck(1) |> round(2) |> as.character())
sr <- map_chr(list(iv2, iv3), ~ compute_delta(iv1, .x, 'fit_log(1 + wealth_defl)', 0.75) |> round(2) |> as.character())
description <- tribble(
...
notes <- "Vote is defined as 1 if the politician is in favor of the reform, 0 otherwise. Robust standard errors clustered at the politician-level in parentheses. Personal Wealth is defined as Log(1+Wealth at Time of Vote), and instrumented by Father's profession."
...
modelsummary(ivres, 
    ...
    kableExtra::save_kable("./tables/iv_results_fisc_log.tex")
