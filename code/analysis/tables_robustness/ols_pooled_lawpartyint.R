...
suffrage_basic <-  feols(vote ~ ihs(wealth_defl) | party_granular^law, cluster=~b1_nummer, data=electoral)
fiscal_basic <- feols(vote ~ ihs(wealth_defl) | party_granular^law, cluster=~b1_nummer, data=fiscal)

# First, main model split up
m1 <- feols(vote ~ ihs(wealth_defl):category | party_granular^law, cluster=~b1_nummer, data=data)
# Suffrage (First three panels)
m2 <- feols(vote ~ ihs(wealth_defl) + industry + services + catholic_pct + hervormd_pct | party_granular^law, cluster=~b1_nummer, data=electoral)
...
m4 <- feols(vote ~ ihs(wealth_defl) + industry + services + catholic_pct + hervormd_pct + 
                ...
                soc_share + soc_dum + vote_share + turnout + amount_of_strikes + dsle
            | party_granular^law, cluster=~b1_nummer, data = electoral)
# Fiscal (Second three panels)
m5 <- feols(vote ~ ihs(wealth_defl) + industry + services + catholic_pct + hervormd_pct | party_granular^law, cluster=~b1_nummer, data=fiscal)
