...
fiscal <- data |> filter(category == 'fiscal', str_detect(law, "Successie"))


# First stage 
fs1 <- feols(ihs(wealth_defl) ~ father_politician | party_granular + law, cluster=~b1_nummer, data = fiscal)
# IV Blank
iv1 <- feols(vote ~ 1 | law + party_granular | ihs(nw0) ~ father_politician, cluster=~b1_nummer, data = fiscal)
# First stage - select controls
fs2 <- feols(ihs(wealth_defl) ~ father_politician + industry + services + catholic_pct + hervormd_pct | party_granular + law, cluster=~b1_nummer, data=fiscal)
# IV - select controls
...
iv3 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct + 
                  tenure + income_tax + vermogen_tax + total_pers_taxes +
                 soc_share + soc_dum + vote_share + vote_share_nc + 
                 turnout + amount_of_strikes + dsle
...
knitr::opts_current$set(label = "ivresults_fisc_nw0_inherit")
...
modelsummary(ivres, 
             ...
             output = "./tables/iv_results_fisc_nw0_inherit.tex",
    ...
    kableExtra::save_kable("./tables/iv_results_fisc_nw0_inherit.tex")
