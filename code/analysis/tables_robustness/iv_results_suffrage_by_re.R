...
# above median 
suffrage_above <- suffrage |>
    filter(re/nw0 > median(re/nw0, na.rm=T))

# below median
suffrage_below <- suffrage |>
    filter(re/nw0 < median(re/nw0, na.rm=T))

# IV Blank
iv1 <- feols(vote ~ 1 | law + party_granular | ihs(wealth_defl) ~ father_politician, cluster=~b1_nummer, data = suffrage_above)
# IV - select controls
iv2 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct | party_granular + law | ihs(wealth_defl) ~ father_politician, cluster=~b1_nummer, data=suffrage_above)
# IV - full controls
iv3 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct + 
             ...
             | party_granular + law | ihs(wealth_defl) ~ father_politician, cluster=~b1_nummer, data = suffrage_above)
# IV Blank
iv4 <- feols(vote ~ 1 | law + party_granular | ihs(wealth_defl) ~ father_politician, cluster=~b1_nummer, data = suffrage_below)
# IV - select controls
iv5 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct | party_granular + law | ihs(wealth_defl) ~ father_politician, cluster=~b1_nummer, data=suffrage_below)
# IV - full controls
iv6 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct + 
                 income_tax + vermogen_tax + total_pers_taxes + tenure + 
                 soc_share + soc_dum + vote_share + vote_share_nc + turnout + amount_of_strikes + dsle
             | party_granular + law | ihs(wealth_defl) ~ father_politician, cluster=~b1_nummer, data = suffrage_below)
...
knitr::opts_current$set(label = "ivresults_suffrage_by_re")
...
modelsummary(ivres, 
             ...
             output = "./tables/iv_results_suffrage_by_re.tex",
             title = "IV Estimates of Wealth on the Propensity to Vote for Suffrage Extensions") |>
    kableExtra::add_header_above(c("Sample:" = 1, "Above Median Re/Wealth" = 3, "Below Median Re/Wealth" = 3)) |>
    kableExtra::kable_styling(latex_options = c("hold_position", "scale_down")) |>
    kableExtra::footnote(general = notes, footnote_as_chunk = T, threeparttable = T, escape = F) |>
    kableExtra::save_kable("./tables/iv_results_suffrage_by_re.tex")
