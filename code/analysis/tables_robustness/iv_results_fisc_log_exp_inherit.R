...
# First stage 
fs1 <- feols(log(1+wealth_defl) ~ expected_inheritance | party_granular + law, cluster=~b1_nummer, data = fiscal)
# IV Blank
iv1 <- feols(vote ~ 1 | law + party_granular | log(1+wealth_defl) ~ expected_inheritance, cluster=~b1_nummer, data = fiscal)
# First stage - select controls
fs2 <- feols(log(1+defw_rb) ~ expected_inheritance + catholic_pct + hervormd_pct + income_tax + vermogen_tax + soc_share + soc_dum + vote_share + vote_share_nc + turnout + dsle
             | party_granular + law, cluster=~b1_nummer, data = fiscal)
# IV - select controls
iv2 <- feols(vote ~ catholic_pct + hervormd_pct + income_tax + vermogen_tax + soc_share + soc_dum + vote_share + vote_share_nc + turnout + dsle
             | party_granular + law | log(1+wealth_defl) ~ expected_inheritance, cluster=~b1_nummer, data = fiscal)
# First stage - full controls
fs3 <- feols(log(1+defw_rb) ~ expected_inheritance + industry + services + catholic_pct + vermogen_tax + vote_share_nc + turnout + dsle  | party_granular + law, cluster=~b1_nummer, data=fiscal)
# IV - full controls
iv3 <- feols(vote ~ industry + services + catholic_pct + vermogen_tax + vote_share_nc + turnout + dsle | party_granular + law | log(1+wealth_defl) ~ expected_inheritance, cluster=~b1_nummer, data=fiscal)
...
modelsummary(ivres, 
             ...
             output = "./tables/iv_results_fisc_log_exp_inherit.tex",
    ...
    kableExtra::save_kable("./tables/iv_results_fisc_log_exp_inherit.tex")
