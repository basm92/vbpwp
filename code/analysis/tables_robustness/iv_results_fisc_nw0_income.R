...
fiscal <- data |> filter(category == 'fiscal', str_detect(law, "Inkom"))
...
fs3 <- feols(ihs(wealth_defl) ~ father_politician + industry + services + catholic_pct + hervormd_pct + 
                 income_tax + vermogen_tax + total_pers_taxes + tenure + turnout + amount_of_strikes + dsle | party_granular + law, cluster=~b1_nummer, data = fiscal)
# IV - full controls
iv3 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct + 
                 tenure + income_tax + vermogen_tax + total_pers_taxes + turnout + amount_of_strikes + dsle
...
knitr::opts_current$set(label = "ivresults_fisc_nw0_income")
...
modelsummary(ivres, 
             ...
             output = "./tables/iv_results_fisc_nw0_income.tex",
    ...
    kableExtra::save_kable("./tables/iv_results_fisc_nw0_income.tex")
