# new_results iv_results_suffrage_otherclassific
...
# First stage 
fs1 <- feols(ihs(wealth_defl) ~ father_politician | party_simple + law, cluster=~b1_nummer, data = electoral)
# IV Blank
iv1 <- feols(vote ~ 1 | law + party_simple | ihs(wealth_defl) ~ father_politician, cluster=~b1_nummer, data = electoral)
# First stage - select controls
fs2 <- feols(ihs(wealth_defl) ~ father_politician + industry + services + catholic_pct + hervormd_pct | party_simple + law, cluster=~b1_nummer, data=electoral)
# IV - select controls
iv2 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct | party_simple + law | ihs(wealth_defl) ~ father_politician, cluster=~b1_nummer, data=electoral)
...
fs3 <- feols(ihs(wealth_defl) ~ father_politician + industry + services + catholic_pct + hervormd_pct + 
                 ...
                 soc_share + soc_dum + vote_share + vote_share_nc + turnout + amount_of_strikes + dsle | party_simple + law, cluster=~b1_nummer, data = electoral)
# IV - full controls
iv3 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct + 
             ...
             | party_simple + law | ihs(wealth_defl) ~ father_politician, cluster=~b1_nummer, data = electoral)
