...
# IV1: First stage 
fs1 <- feols(ihs(wealth_defl) ~ expected_inheritance | party_granular + law, cluster=~b1_nummer, data = data)
# IV1: Blank
iv1 <- feols(vote ~ 1 | law + party_granular | ihs(wealth_defl) ~ expected_inheritance, cluster=~b1_nummer, data = data)
# First stage2: full controls
fs3 <- feols(ihs(wealth_defl) ~ expected_inheritance + industry + services + catholic_pct + hervormd_pct + 
                 income_tax + vermogen_tax + total_pers_taxes + tenure + 
...
iv3 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct + 
             ...
             | party_granular + law | ihs(wealth_defl) ~ expected_inheritance, cluster=~b1_nummer, data = data)
...
notes <- "Vote is defined as 1 if the politician is in favor of the reform, 0 otherwise. Robust standard errors clustered at the politician-level in parentheses. Personal Wealth is defined as ihs(Wealth at Time of Vote), and if applicable, instrumented by Expected Inheritance."
