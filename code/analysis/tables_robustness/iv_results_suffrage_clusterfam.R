...
# First stage 
fs1 <- feols(ihs(wealth_defl) ~ father_politician | party_granular + law, cluster=~pdc_naam, data = electoral)
# IV Blank
iv1 <- feols(vote ~ 1 | law + party_granular | ihs(wealth_defl) ~ father_politician, cluster=~pdc_naam, data = electoral)
# First stage - select controls
fs2 <- feols(ihs(wealth_defl) ~ father_politician + industry + services + catholic_pct + hervormd_pct | party_granular + law, cluster=~pdc_naam, data=electoral)
# IV - select controls
iv2 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct | party_granular + law | ihs(wealth_defl) ~ father_politician, cluster=~pdc_naam, data=electoral)
...
fs3 <- feols(ihs(wealth_defl) ~ father_politician + industry + services + catholic_pct + hervormd_pct + 
                 ...
                 soc_share + soc_dum + vote_share + vote_share_nc + turnout + amount_of_strikes + dsle | party_granular + law, cluster=~pdc_naam, data = electoral)
# IV - full controls
iv3 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct + 
             ...
             | party_granular + law | ihs(wealth_defl) ~ father_politician, cluster=~pdc_naam, data = electoral)
...
notes <- "Vote is defined as 1 if the politician is in favor of the reform, 0 otherwise. Robust standard errors clustered at the political family-level in parentheses. Personal Wealth is defined as ihs(Wealth at Time of Vote), and instrumented by Father's profession."
knitr::opts_current$set(label = "ivresults_suffrage_clusterfam")
...
modelsummary(ivres, 
             ...
             output = "./tables/iv_results_suffrage_clusterfam.tex",
    ...
    kableExtra::save_kable("./tables/iv_results_suffrage_clusterfam.tex")
