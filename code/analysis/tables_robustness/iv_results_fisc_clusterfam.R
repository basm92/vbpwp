...
# First stage 
fs1 <- feols(ihs(wealth_defl) ~ father_politician | party_granular + law, cluster=~pdc_naam, data = fiscal)
# IV Blank
iv1 <- feols(vote ~ 1 | law + party_granular | ihs(wealth_defl) ~ father_politician, cluster=~pdc_naam, data = fiscal)
# First stage - select controls
fs2 <- feols(ihs(wealth_defl) ~ father_politician + industry + services + catholic_pct + hervormd_pct | party_granular + law, cluster=~pdc_naam, data=fiscal)
# IV - select controls
iv2 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct | party_granular + law | ihs(wealth_defl) ~ father_politician, cluster=~pdc_naam, data=fiscal)
...
fs3 <- feols(ihs(wealth_defl) ~ father_politician + industry + services + catholic_pct + hervormd_pct + 
                 ...
                 soc_share + soc_dum + vote_share + vote_share_nc + turnout + amount_of_strikes + dsle | party_granular + law, cluster=~pdc_naam, data = fiscal)
# IV - full controls
iv3 <- feols(vote ~ industry + services + catholic_pct + hervormd_pct + 
             ...
             | party_granular + law | ihs(wealth_defl) ~ father_politician, cluster=~pdc_naam, data = fiscal)
...
knitr::opts_current$set(label = "ivresults_fisc_clusterfam")
...
modelsummary(ivres, 
             ...
             output = "./tables/iv_results_fisc_clusterfam.tex",
    ...
    kableExtra::save_kable("./tables/iv_results_fisc_clusterfam.tex")